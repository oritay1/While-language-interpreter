...
pub fn solve_b(e: &BExp, s: &State) -> BVal {
    match e {
        BExp::True => String::from("tt"),

        BExp::False => String::from("ff"),
        ...
        BExp::Beq(e1, e2) => {
            if solve_b(e1, s) == solve_b(e2, s) { String::from("tt") } else { String::from("ff") }
        },

        BExp::Aeq(e1, e2) => {
            if solve_a(e1, s) == solve_a(e2, s) { String::from("tt") } else { String::from("ff") }
        },
        ...
        BExp::And(e1, e2) => {
            if solve_b(e1, s) == "tt" && solve_b(e2, s) == "tt" { String::from("tt") } else { String::from("ff") }
        },
    }
}
